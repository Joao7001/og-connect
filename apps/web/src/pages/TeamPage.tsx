...
export default function TeamPage() {
  ...
  useEffect(() => {
    api.list("members").then(setRecords).catch(() => setRecords([]));
  }, []);
  ...
  return (
    <section className="container page">
      <SectionHeading eyebrow="PESSOAS QUE FAZEM" title="A nossa equipe" text="Cada voz, uma perspectiva. Cada criador, uma parte da história." />
      <div className="member-grid member-showcase">
        {currentMembers.map((member) => (
          <Link to={`/equipe/${member.id}`} className="member-card" key={member.id}>
            ...
            <div className="member-socials">
              {member.socials.slice(0, 6).map((social) => (
                <span key={socialData(social).network}><SocialIcon network={socialData(social).network} official /></span>
              ))}
            </div>
          </Link>
        ))}
